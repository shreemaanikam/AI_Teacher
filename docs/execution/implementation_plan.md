...
| --- | --- | --- | --- |
| 0 | Repository, agent, and local runtime foundation | IP-001–003, IP-017, IP-021 | App boots; contracts/orchestrator/runtime and CI pass |
...
| --- | --- | --- | --- |
| IP-001 | PLANNED | Scaffold Flask factory, React/Vite TypeScript client, configuration, containers, health endpoints | — |
...
| IP-007 | PLANNED | Versioned prompt registry, local model-runtime port, structured plan schema, time/level/language planning | IP-003,006,021 |
...
| IP-015 | PLANNED | Load/resilience/compute controls, local model/backend fallbacks, backup/restore and deletion verification | IP-014,023 |
...
| IP-021 | PLANNED | Pin/build llama.cpp CUDA, Vulkan, and CPU artifacts; implement hardware probe, Model Runtime Gateway, health, admission, and residency policy | IP-001–003 |
| IP-022 | PLANNED | Implement Hugging Face allowlist/download quarantine, GGUF manifest verification, license consent, offline import, and atomic activation | IP-002,021 |
| IP-023 | PLANNED | Benchmark capability models/quantizations on NVIDIA CUDA, targeted Intel Vulkan, and CPU; publish hardware matrix and evaluation gates | IP-021–022 |
| IP-024 | PLANNED | Build reproducible local fine-tuning, adapter/merge, GGUF conversion/quantization, model-card, and registry-promotion pipeline | IP-022–023 |

## Increment details

### Phase 0 — foundation

Build a vertical “hello lesson” slice through browser, API, orchestrator, fake specialist, local llama.cpp gateway, artifact validator, database, queue, worker, and event stream. Establish versioned agent/runtime contracts before integrating real models. Exit only when one command starts local dependencies and CI runs format, lint, types, unit, integration, agent/runtime conformance, frontend, and migration checks.
...
Implement Visual Planner/Renderer, Voice, Avatar, Composer, and Accessibility specialists. Render exact visuals locally first. Add neural voice/avatar only when the pinned llama.cpp build supports an evaluated GGUF; otherwise use deterministic local tools and accessibility fallback. Exit with checkpoint-paused playback, captions/transcript, keyboard controls, and forced media-agent/runtime failure fallback.
...
- New models have an approved manifest, license, immutable source/lineage, checksum, GGUF/runtime compatibility, backend-specific evaluation, memory budget, and no hosted inference dependency.
...
| Risk | Mitigation |
| --- | --- |
| Local model latency/resource exhaustion | Quantized profiles, admission control, shared residency, precomputation, smaller-model/CPU fallback |
...
| Multilingual quality | Native-speaker review set, local model/quantization/backend matrix, fallback disclosure |
| Intel/Vulkan variability | Tested device/driver matrix, startup probes, tuned context/concurrency, CPU fallback |
| Malicious/tampered model | Allowlist, pinned revision, quarantine, hash/license/metadata checks, atomic activation, isolated serving |
| llama.cpp media incompatibility | Do not add a second neural runtime; use supported GGUF only or deterministic accessible fallback |
