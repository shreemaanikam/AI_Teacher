...
| --- | --- | --- | --- | --- |
| TD-001 | OPEN | Concrete providers/models for each capability are not selected or benchmarked. | Quality, latency, availability, multilingual behavior, and cost are uncertain. | Evaluate approved configurations and failover behavior during IP-023. |
...
| TD-016 | OPEN | Third-party provider retention, residency, deletion, and subprocessor terms are not selected. | Learner/source data may cross an unacceptable privacy or compliance boundary. | Complete the disclosure and provider governance review in IP-024 before external use. |
| TD-017 | OPEN | High-quality multilingual TTS/avatar/video providers and fallbacks are not selected. | Media quality, lip sync, language coverage, latency, and cost may miss the demo target. | Evaluate short segments early and retain typed, transcript, caption, and deterministic visual fallbacks. |
| TD-018 | OPEN | Provider cost and quota behavior under a complete lesson is unmeasured. | A demo or production burst could exhaust limits or create unexpected spend. | Add per-capability budgets, usage telemetry, caching, alerts, and load rehearsal before release. |
| TD-019 | OPEN | Cross-provider quality drift is unmeasured. | Failover may alter grading, grounding, safety, or multilingual quality. | Maintain capability-specific evaluation gates and prohibit silent fallback until equivalence is demonstrated. |
